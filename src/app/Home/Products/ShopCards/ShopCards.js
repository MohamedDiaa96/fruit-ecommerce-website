                ...
                <div className="flex items-center">
                    ...
                    <div className="w-[182.44000244140625px] h-[35px] opacity-100 rounded-[5px] bg-[#FFFFFF] border border-solid border-[#E9E9E9] flex items-center justify-center">
                        <span className="flex items-center font-poppins font-normal text-[15px] leading-[18px] tracking-[0%] align-middle text-[#7A7A7A] gap-2">
                            ...
                            <div className={`${sortingdropdownmenu === true ? "z-20 shadow-xl transform transition-all duration-300 scale-y-100 visible max-h-fit  origin-top" : "transform transition-all scale-y-0 duration-300 invisible  origin-top"} overflow-hidden absolute mt-28 ml-13 rounded-[10px] w-[100px] h-20 bg-white border border-solid border-[#E9E9E9]  `}>
                                {sortingooptions.map((s, index) => (<button key={index} onClick={() => { setselectedsortoption(s); setsortingdropdownmenu(false) }} className="flex py-1 flex-col items-center w-full border-b-1 cursor-pointer hover:text-white min-sm:max-xl:focus:text-white min-sm:max-xl:focus:bg-[#F53E32] hover:bg-[#F53E32]">{s}</button>
                                ))}
                            </div>
                        </span>
                    </div>
