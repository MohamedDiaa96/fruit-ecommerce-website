  ...
  return (
    <> {cardtype === "A" ?
      <Link href={`/Home/Products/${id}`} className=" w-[265px] h-[475.3800048828125px] flex flex-col border border-[#ECECEC] rounded-[15px]">
        ...
        <div className="flex">
          {/* card tag */}
          <div className={`w-[60.22999954223633px] h-[31px] rounded-tl-[15px] rounded-br-[20px] flex justify-center items-center ${tag === "Hot" ? "bg-[#F74B81]" : tag === "Sale" ? "bg-[#67BCEE]" : tag === "New" ? "bg-[#3BB77E]" : tag === "-14%" ? "bg-[#F59758]" : ""}`} ><h1 className='text-white font-lato font-normal text-[12px] leading-3 align-middle'>{tag}</h1></div>
        </div>
        <div>
          {/* image */}
        ...
        <Link href={`/Home/Products/${id}`} className="w-[225px] h-full rounded-[15px] mb-4 flex flex-col border border-[#ECECEC] ">
          ...
          <div className="flex">
            {/* card tag */}
            <div className={`w-[30%] h-[31px] rounded-tl-[15px] rounded-br-[20px] flex justify-center items-center ${tag === "Save 15%" ? "bg-[#F74B81]" : tag === "Sale" ? "bg-[#67BCEE]" : tag === "Save 35%" ? "bg-[#3BB77E]" : tag === "Best sale" ? "bg-[#F59758]" : ""}`} ><h1 className='text-white font-lato font-normal text-[12px] leading-3 align-middle'>{tag}</h1></div>
          </div>
          <div>
            {/* image */}
            <Image src={img} width={246} height={246} alt={title}></Image>
          ...
          <div className="mx-5 flex flex-col gap-2 mb-4">
            ...
            <div>
              {/* progress bar */}
              <progress className="bg-[#E9ECEF] w-full h-[5px] rounded-lg [&::-moz-progress-bar]:bg-[#F53E32]" value="50" max="100"> 32% </progress>
            </div>
            <div>
              {/* sold items value */}
            ...
            <div>
              {/* add to cart button */}
              <ButtonUI buttontype={"A"} text={"Add To Cart"} onclick={(e) => { e.preventDefault(); e.stopPropagation(); handleaddtocart() }} />
            </div>
          </div>
        </Link> : cardtype === "C" ?
            ...
            <Link href={`/Home/Products/${id}`} className="w-full h-full flex rounded-[15px] justify-center ">
              ...
              <Link href={`/Home/Products/${id}`} className="sm:max-[734px]:w-[80%] sm:max-[734px]:h-130  w-[306px] h-[477px] items-center bg-[#FFFFFF] flex flex-col border border-[#E9E9E9] rounded-[5px]">
                ...
                <div className="w-[280px] sm:max-[734px]:w-[80%]  h-[280px] border my-[13px] border-[#E9E9E9] rounded-[5px]">
                  {/* image container */}
                  <Image src={img} width={278} height={278} alt={title} className="sm:max-[734px]:h-full"></Image>
                  <div className="flex justify-center border-[#E9E9E9]  sm:max-[734px]:justify-center z-10 ">
                    {/* cart bag circle box container */}
                    <button className="cursor-pointer sm:max-[734px]:w-15 sm:max-[734px]:h-15 z-10 group hover:bg-[#88d692] transition-all duration-300 flex justify-center w-10 h-10 border -my-5 bg-[#F7F7F8] border-[#E9E9E9] items-center rounded-4xl" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleaddtocart() }}>
                      {/* cart bag circle box */}
                      <svg width="12" height="14" viewBox="0 0 13 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <section className="my-7 gap-2 flex flex-col  sm:max-[734px]:justify-center">
                    {/* product card details */}
                    ...
                    <div className="flex w-full justify-center items-center gap-1.5">
                      ...
                      <div>
                        {/* rating number */}
                        <span className="font-poppins text-[#999999] font-normal text-[11px] leading-2.5 tracking-[0.48px] text-center align-middle">({ratingnumber})</span>
                      </div>
                    </div>
                    <div>
                      {/* title */}
                      <h1 className="font-poppins text-[#2B2B2D] font-medium text-[15px] leading-6 tracking-[0.48px] text-center align-middle">{title}</h1>
                    </div>
                    <div className="flex gap-2 justify-center items-center">
                      {/* prices */}
                      <span className="font-poppins font-bold text-[16px] leading-7 tracking-[0.48px] text-center align-middle text-[#F53E32]">${discountedprice}</span>
