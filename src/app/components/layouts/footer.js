            ...
            <div className="flex flex-col min-sm:max-xl:flex-wrap min-sm:max-xl:w-full min-sm:max-xl:ml-[10%]">
                ...
                <div className="flex gap-16 min-sm:max-xl:flex-wrap min-sm:max-xl:w-full">
                    ...
                    <div className=" h-[43vh] flex flex-col items-start gap-[17.4px] mt-8 ">
                        ...
                        <div>
                            {/* input */}
                            <div className="flex  bg-white w-[316px] h-[44px] pl-4 border-1 border-[#00000025] rounded-[4px]"><input type="text" className="placeholder:text-[#7A7A7A] placeholder:font-poppins placeholder:font-normal placeholder:text-sm placeholder:tracking-normal placeholder:align-middle outline-0 bg-white w-[87%]" placeholder="Search Here..."></input><Image src="/material-photos/Footer/arrow.svg" alt="arrow" width={21} height={21}></Image></div>
                        </div>
                        <div className="flex gap-[4px]">
                            {/* link icons */}
                            {iconlinks.map((icons, index) => (<Link href={icons.url} key={index} className={`${ index === 0 ? "hover:bg-blue-600 transition-all duration-300":""} ${ index === 1 ? "hover:bg-black transition-all duration-300":""} ${ index === 2 ? "hover:bg-blue-400  transition-all duration-300":""} ${index === 3 ? "transition-all duration-300 hover:bg-gradient-to-r hover:from-[#833ab4] hover:via-[#fd1d1d] hover:to-[#fcb045]":""} bg-white border-1 border-solid border-[#E1DFDF] w-[35px] h-[35px] rounded-[5px] flex justify-center`}>
                                <Image src={icons.icon} alt="icon" width={icons.width} height={icons.height} className={`${ index === 0 ? "hover:invert-100 transition-all duration-300":""} ${ index === 1 ? "hover:invert-100 transition-all duration-300":""} ${ index === 3 ? "hover:invert-100 transition-all duration-300":"hover:invert-100 transition-all duration-300"} `}></Image>
                            </Link>))}
                        </div>
