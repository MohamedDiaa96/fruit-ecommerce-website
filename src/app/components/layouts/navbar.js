        ...
        <div>
            <div className="flex items-center h-[9vh] w-full shadow-[0px_0px_5px_0px] shadow-[#0000003f] min-sm:max-[738px]:gap-10 min-sm:max-xl:flex-wrap min-sm:max-xl:h-[100%] min-sm:max-xl:w-[100%] min-sm:max-xl:items-center min-sm:max-xl:justify-center min-sm:max-xl:flex-col">
                ...
                <div className="flex items-center h-[10vh] w-[10%] ml-[212px] min-sm:max-xl:ml-0 min-sm:max-xl:w-[100%] min-sm:max-xl:flex-wrap min-sm:max-xl:justify-center">
                    ...
                    <button onClick={() => { setopensidebar(true) }}><Image src="/material-photos/Navbar/div.cr-category-toggle.svg" alt="category" width={35} height={35} className="cursor-pointer w-auto h-auto rounded-md hover:bg-green-400 min-sm:max-xl:focus:bg-green-400"></Image></button>
                    ...
                    <div className={`w-[250px] h-screen gap-3 bg-white shadow-lg z-50 fixed top-0 left-0 flex flex-col p-5 transform transition-transform duration-300 ${opensidebar ? "translate-x-0" : "-translate-x-full"}`}>
                        ...
                        {navlinks.map((link, index) => (
                            <div key={index} className="flex w-fit ml-17 gap-1" >
                                {/* page links title */}
                                {link.path ? (<Link href={link.path} className="hover:text-[#F53E32] min-sm:max-xl:focus:text-[#F53E32] gap-10 transition-all ease-in-out font-poppins flex font-[600] text-[14px] leading-[21px] tracking-[0.48px] align-middle">
                                    {link.url} </Link>) : (<button onClick={() => { setdropdownmenusidebar(dropdownmenusidebar === link.name ? null : link.name) }} onMouseEnter={() => { setdropdownmenusidebar(dropdownmenusidebar === link.name ? null : link.name) }} className="hover:text-[#F53E32] min-sm:max-xl:focus:text-[#F53E32] transition-all cursor-pointer ease-in-out font-poppins flex font-[600] text-[14px] leading-[21px] tracking-[0.48px] align-middle">
                ...
                <div className="flex items-center h-[10vh] w-[50%] gap-[55.66px] min-sm:max-xl:flex-wrap min-sm:max-xl:gap-15 min-sm:max-xl:justify-center min-sm:max-xl:w-[100%]">
                    ...
                    {navlinks.map((link, index) => (
                        <div key={index} >
                            {/* page links title */}
                            {link.path ? (<Link href={link.path} className="hover:text-[#F53E32]  min-sm:max-xl:focus:text-[#F53E32]transition-all ease-in-out font-poppins flex font-[600] text-[14px] leading-[21px] tracking-[0.48px] align-middle">
                                {link.url} </Link>) : (<button onClick={() => { setdropdownmenu(dropdownmenu === link.name ? null : link.name) }} className="hover:text-[#F53E32] min-sm:max-xl:focus:text-[#F53E32] transition-all cursor-pointer ease-in-out font-poppins flex font-[600] text-[14px] leading-[21px] tracking-[0.48px] align-middle">
            ...
            <div className="flex items-center h-[12vh] w-[100%] border-b-2 border-b-[#00000024]  min-sm:max-xl:h-[30vh] min-sm:max-xl:w-[100%] min-sm:max-xl:flex-wrap min-sm:max-xl:flex-col">
                    ...
                    <div className="flex">
                        <div className="flex flex-col">
                            ...
                            {filtereditem.length > 0 && (<div className="bg-white max-h-[252px] w-fit  origin-top rounded-lg shadow-lg absolute mt-10 border border-[#0000004f] z-30 overflow-y-auto">
                                {filtereditem.map((p) => (<Link key={p.id} href={`/Products/${p.id}`} className="hover:bg-[#F53E32] min-sm:max-xl:focus:bg-[#F53E32] pr-5 gap-2 items-center  min-sm:max-xl:focus:text-white hover:text-white border border-[#0000004f] flex">
                ...
                <div className="flex justify-center min-sm:max-xl:items-center min-sm:max-xl:w-full gap-8 min-sm:max-xl:h-[20%]">
                    ...
                    {navinternallinks.map((internallinks, index) => (<div key={index} className="flex text-[15px] font-poppins font-[500] group min-sm:max-xl:focus:text-[#F53E32] hover:text-[#F53E32] transition-all duration-200  ">
                        {/* account,wishlist,Cart div box */}
                        <Link href={internallinks.url}><div className="flex gap-2 items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="..." style={{ width: internallinks.width, height: internallinks.height }} className="fill-black group-hover:fill-[#F53E32] group-min-sm:max-xl:focus:fill-[#F53E32] transition-all duration-300">
