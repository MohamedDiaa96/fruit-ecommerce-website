...
export default function Navbar() {
    ...
    {
        name: "Products", products: [{ link: "All Products", path: "/Products" },
                                        ...
                                        if (Array.isArray(value)) {
                                            return (
                                                <div key={key} ref={dropdownRef} className={`${dropdownmenusidebar === link.name ? " fixed scale-x-100 visible origin-left max-h-fit opacity-100  flex flex-col transform transition-transform duration-300 w-[150px] bg-white shadow-lg z-30 " : " fixed w-[150px] bg-white shadow-lg z-30 transform transition-transform duration-300 flex flex-col opacity-100 scale-x-0 visible max-h-0 origin-left"} `}>
                                                    {/* drop down menu  */}
                                                    {value.map((item, index) => (<Link className="border-1 pl-2 hover:bg-[#F53E32] hover:text-white font-poppins border-[#0000004f] font-[600] text-[14px]  leading-[21px] tracking-[0.48px] align-middle" key={index} href={item.path}>{item.link}</Link>))}
                                                    {/* drop down links */}
                                                </div>)
                                        }
                                    })}
                ...
                <div className="flex justify-center min-sm:max-xl:items-center min-sm:max-xl:w-full gap-8 min-sm:max-xl:h-[20%]">
                    ...
                    {navinternallinks.map((internallinks, index) => (<div key={index} className="flex  text-[15px] font-poppins font-[500] group  hover:text-[#F53E32] transition-all duration-200  ">
                        ...
                        <Link href="/"><div className="flex gap-2 items-center">
                            ...
                            </svg>                            <span >{internallinks.text}</span>
                        </div></Link>
                    </div>))}
                </div>
            </div>
